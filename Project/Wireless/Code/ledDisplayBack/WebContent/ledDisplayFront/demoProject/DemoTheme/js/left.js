$(function () {
    var clientId = 'left' + parseInt(100 * Math.random()); // 可以改自己喜欢的名字，不冲突就行，最好是页面的名字
        ...
        if (msg.msgReceiver == 'All') {//群发消息
            if (msg.msgType == 'changeTheme') {
                if (msg.msgValue == 'theme1') {
                    $(window).attr('location', self_href + 'demoProject/DemoTheme/html/leftDemo.html');
                } else if (msg.msgValue == 'theme2') {
                    $(window).attr('location', self_href + 'demoProject/DemoTheme/html/leftDemo2.html');
                }
            }
...
function getSolrData(fireWallSolrUrl, keyInfo, dataInfo) {
    $.ajax({
        url: ajaxUrl + '/servlet/DemoServlet',
        type: 'POST',
        dataType: "JSONP",
...
function show(data) {
    $('.leftTitle').append(' 屏幕位置：' + data.location + '<br /> ' + data.today);
}
