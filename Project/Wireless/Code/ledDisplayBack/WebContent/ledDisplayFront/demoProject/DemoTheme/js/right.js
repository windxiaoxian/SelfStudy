$(function () {
    var clientId = 'right' + parseInt(100 * Math.random()); // 可以改自己喜欢的名字，不冲突就行，最好是页面的名字
        ...
        if (msg.msgReceiver == 'All') {//群发消息
            if (msg.msgType == 'changeTheme') {
                if (msg.msgValue == 'theme1') {
                    $(window).attr('location', self_href + 'demoProject/DemoTheme/html/rightDemo.html');
                } else if (msg.msgValue == 'theme2') {
                    $(window).attr('location', self_href + 'demoProject/DemoTheme/html/rightDemo2.html');
                }
            }
    ...
    getData(ajaxUrl, 'right', dataInfo);
});
...
function show(data) {
    $('.rightTitle').append(' 屏幕位置：' + data.location + '<br /> ' + data.today);
}
