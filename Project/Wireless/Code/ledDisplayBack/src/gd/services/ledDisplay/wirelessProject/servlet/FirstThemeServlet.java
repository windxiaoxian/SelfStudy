...
import gd.services.ledDisplay.wirelessProject.dao.FirstThemeDao;
   ...
   private String getBroadCastTimebyMonth(JSONObject json,String screenPosition){
        try{
            ...
            for(int i=0;i<BroadTimeList.size();i++) {
                ...
                if( i>=currMonthIndex&& i<12){
                    if("DW".equals(broadCastItem.get("order_type"))) {
                        broadCastExterTime += (int)broadCastItem.get("total_hour");
                    }else if("DN".equals(broadCastItem.get("order_type"))){
                        broadCastInterTime += (int)broadCastItem.get("total_hour");
                    }else  if("SY".equals(broadCastItem.get("order_type"))){
                        broadCastTestTime += (int)broadCastItem.get("total_hour");
                    }
                }
                ...
                if (i<12) {
                    ...
                    if (currMonth.equals(broadCastItem.get("month"))) {
                        for (int j = 0; j < TodayBroadCastList.size(); j++) {
                            todayBroadTime += (int)((Map)TodayBroadCastList.get(i)).get("total_hour");
                        }
                        temMap.put("BroadTime", (int)broadCastItem.get("total_hour") + (int)((Map)BroadTimeList.get(i + 12)).get("total_hour") + (int)((Map)BroadTimeList.get(i + 12)).get("total_hour") + todayBroadTime);
                    } else {
                        temMap.put("BroadTime", (int)broadCastItem.get("total_hour") + (int)((Map)BroadTimeList.get(i + 12)).get("total_hour") + (int)((Map)BroadTimeList.get(i + 12)).get("total_hour"));
                    }
       ...
       try{
           ...
           List taskCountList = (List) wirelessDB.qryList(FrontPageThemeDAO.getTodayTotalTasks(reqMap)).get("data"); //每种任务对应的在播频率数
              for(Object taskCount : taskCountList){
               ...
               map.put("taskCount",(int)taskCountItem.get("task_done")+(int)taskCountItem.get("task_todo")+(int)taskCountItem.get("task_doing"));//任务总数
               resultList.add(map);
           }
//           for(int i=0;i<3;i++){
