...
//           Map PassPrecentMap = (HashMap)ledDisplayDB.qryMap(FirstThemeDao.getAudiBility(reqMap)).get("data");//年度实验合格率
    ...
    protected  String getSubStationInfo(JSONObject json,String screenPosition){
        try{
            ...
            String room = ((HashMap)machineList.get(0)).get("room").toString();//当前遍历机房号
            String workStatus = "";//当前该发射机的状态
            List transList = new ArrayList();
            for(Object  machineItem : machineList){
                Map machineMap = (Map)machineItem;
                //计算所有机房的发射机的各种状态数据汇总
                if(machineMap.get("work_status").equals("正常")){
                        if(realTimePower==-1 || realTimeAM==-1 ||"".equals(transmitterName)) {//  发射机当前状态 : 取其中一条正常的数据展示
                            ...
                            realTimePower = (int)Math.ceil((float)machineMap.get("power")/10);
                            realTimeAM = (int)Math.ceil((float)machineMap.get("AMRP")/10);
                        }
                    workStatus = "Normal";
                ...
                if(room.equals(machineMap.get("room").toString())){//同一个机房
                    ...
                    if(transList.size()<5){ //如果一个机房的机号不足5个,补充null
                        for(int i=transList.size();i<5;i++){
                            transList.add("null");
                        }
                    }
                    machineList.add(transList);//每隔机房返回一个List,list中只显示接收机状态;
            ...
            resultJson.accumulate("planTask",reqMap.get("task_todo"));//计划任务
            resultJson.accumulate("stopTime",reqMap.get("stop_time"));//停播时长
            resultJson.accumulate("planBroadTime",reqMap.get("play_time_todo"));//计划播出时长
            resultJson.accumulate("broadTimeDone",reqMap.get("play_time_done"));//已播出时长
            resultJson.accumulate("broadTaskCount",reqMap.get("task_done"));//已播出任务数
            resultJson.accumulate("alarmCount",reqMap.get("alarm_count"));//综合报警数

            resultJson.accumulate("dispatchOrder",reqMap.get("busiorder_count"));//今日调度令个数
            resultJson.accumulate("broadCastPowerOn",reqMap.get("power_doing"));//实时播出功率
            ...
            resultJson.accumulate("currentPower",(int)Math.ceil((float)realTimePower/10));//当前发射机功率
            resultJson.accumulate("realTimeAM",(int)Math.ceil((float)realTimeAM/10));//调幅度
            resultJson.accumulate("transmitterName",transmitterName);

            resultJson.accumulate("machineList",machineList);//机房状态
