...
public class demoDB {
    private static final Logger logger = LogManager.getLogger(demoDB.class.getName());

    private static DataSource demoDB;
    ...
    static {
        ...
        try {
            ...
            properties.load(demoDB.class.getClassLoader().getResourceAsStream("dataSource.properties"));
            for (Object key : properties.keySet()) {
                String temp = (String) key;
                if (temp.startsWith("jdbc.demoDB.")) {
            ...
            demoDB = getDruidDataSource(dbProperties);
        }
    }
    ...
    public static Map<String, Object> qryList(String sql, Object... params) {
        ...
        resp.put("code", "0000");
        try {
            QueryRunner run = new QueryRunner(demoDB);
            resp.put("data", run.query(
                    sql, new MapListHandler(), params));
        } catch (Exception e) {            //记录日志
            logger.error("[demoDB.qryList]后台报错" + e.getMessage());
            resp.put("code", "9999");
        }
        return resp;
    }
    ...
    public static Map<String, Object> qryMap(String sql, Object... params) {
        ...
        try {
            ...
            logger.error("[demoDB.qryMap]后台报错" + e.getMessage());
            resp.put("code", "9999");
        }
        return resp;
    }
