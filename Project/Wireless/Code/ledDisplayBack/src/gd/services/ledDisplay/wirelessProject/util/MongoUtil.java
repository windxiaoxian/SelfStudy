...
//        System.out.println("===============MongoDBUtil初始化========================");
        ...
        try {
            properties.load(MongoUtil.class.getClassLoader().getResourceAsStream("dataSource.properties"));

            for (Object key : properties.keySet()) {
                String temp = (String) key;
                if (temp.startsWith("MongoDB.")) {
                    String name = temp.substring(8);
                    paramMap.put(name, properties.getProperty(temp));
                }
            }
        } catch (Exception e) {
            paramMap.put("ip", "10.6.3.166");
            paramMap.put("port", 27017);
        }

        instance.mongoClient = new MongoClient((String) paramMap.get("ip"), Integer.parseInt((String) paramMap.get("port")));
     ...
     * 条件查询
     */
    public MongoCursor<Document> find(MongoCollection<Document> coll, Bson queryFilter, Bson sortFilter, int limitFilter, int skipFilter) {
        return coll.find(queryFilter).sort(sortFilter).limit(limitFilter).skip(skipFilter).iterator();
    }
